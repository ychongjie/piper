...
// 解析标准模型名(model 省略=default_model)——日志里标到底是哪个模型在说话,便于 benchmark。
function modelName(model?: string): string {
  try { return resolveModel(model).modelName ?? model ?? "?"; } catch { return model ?? "default"; }
}
...
// tag 形如 "执行·deepseek-v4-pro" / "归因#2·glm-5.2":每行都带上,grep `[tag]` 即可分离某 agent/某模型的输出。
function observe(session: any, log: (m: string) => void, tag: string): () => void {
  const p = `    [${tag}] `;
  if (typeof session?.subscribe !== "function") return () => {};
  return session.subscribe((ev: any) => {
    try {
      if (ev.type === "tool_execution_start") log(`${p}⎯ ${ev.toolName}: ${brief(ev.args?.command ?? ev.args?.path ?? "", 220)}`);
      else if (ev.type === "tool_execution_end") log(`${p}⎿ ${ev.isError ? "✗" : "✓"} ${brief(textOf(ev.result), ev.isError ? 800 : 300)}`);
      else if (ev.type === "message_end") { const t = brief(textOf(ev.message)); if (t) log(`${p}· ${t}`); }
    } catch {}
  });
}

// ── 活叶子:起一个 pi 会话,注入 tools 文档 + 上下文,续会话直到 verify 过 / budget ──
async function runLeaf(leaf: LeafNode, ctx: RunCtx, role = "agent"): Promise<NodeResult> {
  const cwd = expandHome(leaf.cwd) ?? ctx.cwd;
  const isJudge = !!leaf.labels?.length;
  const tag = `${role}·${modelName(leaf.model)}`; // 日志身份:角色·模型
  ...
  const unobserve = observe(session, ctx.onLog, tag);
  ...
  try {
    for (let round = 1; round <= budget; round++) {
      ...
      ctx.onLog(`  [${tag}] 第 ${round}/${budget} 轮……`);
      await session.prompt(prompt);
      if (!captured) { lastVerify = "(没 submit 结果)"; continue; }
      if (isJudge) { ctx.onLog(`  [${tag}] 结论=${captured.label}(${captured.confidence})`); return { label: captured.label, result: JSON.stringify(captured), status: "done", raw: captured }; }
      // 干活:跑 verify
      const vres = await runVerify(leaf.verify, captured.result, ctx, leaf.model);
      if (vres.ok) { ctx.onLog(`  [${tag}] ✓ 验收通过`); return { result: captured.result, status: captured.status, raw: captured }; }
      lastVerify = vres.output;
      ctx.onLog(`  [${tag}] ✗ 验收未过:${brief(vres.output, 200)}`);
...
async function runVerify(
  ...
  const un = observe(session, ctx.onLog, `验收·${modelName(model)}`);
  try {
...
async function runFanout(node: FanoutNode, ctx: RunCtx): Promise<NodeResult> {
  ...
  const models = f.over.map((ov) => modelName(ov.model)); // 每份用的模型(benchmark 对照)
  const branches = f.over.map((ov, i) => async (): Promise<NodeResult> => {
    ...
    ctx.onLog(`  fanout 份 ${i + 1}/${f.over.length} 模型=${models[i]}……`);
    return runLeaf(leaf, ctx, `归因#${i + 1}`);
  });
  // 并发上限
  ...
  // 每份(模型→标签)逐条记下,便于横评不同模型的归因
  results.forEach((r, i) => ctx.onLog(`  归因#${i + 1}(${models[i]}) → ${r.label ?? `(无:${brief(r.result, 60)})`}`));
  if (f.gather.how === "vote") {
    ...
    const perModel = results.map((r, i) => `${models[i]}=${r.label ?? "?"}`).join(", ");
    ctx.onLog(`  fanout vote:票数=${JSON.stringify(tally)} → ${win}(逐模型:${perModel})`);
    return { label: win, result: `vote=${win} 票数=${JSON.stringify(tally)} 逐模型[${perModel}]\n` + results.map((r, i) => `【归因#${i + 1} ${models[i]}】\n${r.result}`).join("\n---\n"), status: "done" };
  }
  // merge:派一个合并者
...
// 给节点推个可读角色名(纯日志用,不进 YAML 词汇):执行/分诊/归因,否则按位置。
function roleOf(node: Node, i: number): string {
  if (isFanout(node)) return "归因";
  if (isLeaf(node)) {
    if (node.tools?.length) return "执行";
    if (node.labels?.length) return "分诊";
  }
  return `step${i + 1}`;
}
...
async function runList(list: Node[], ctx: RunCtx): Promise<NodeResult> {
  ...
  for (let i = 0; i < list.length; i++) {
    const r = await runNode(list[i], ctx, roleOf(list[i], i));
    ctx.prior = r;
...
export async function runNode(node: Node, ctx: RunCtx, role = "agent"): Promise<NodeResult> {
  if (typeof node === "string") return runLeaf({ intent: node }, ctx, role);
  ...
  return runLeaf(node as LeafNode, ctx, role);
}
